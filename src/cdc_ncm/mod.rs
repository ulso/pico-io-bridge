//! CDC-NCM, with the one descriptor byte macOS needs.
//!
//! A copy of `embassy_usb::class::cdc_ncm` from embassy-usb 0.6.0, byte for
//! byte, apart from three things: `bmNetworkCapabilities` is 1 rather than 0
//! (see the comment at the descriptor, which is the whole point of the copy),
//! the four `use crate::` imports now name `embassy_usb::`, and the private
//! `info!`/`warn!` macros now name `defmt::` directly.
//!
//! Copied rather than forked because the class uses only embassy-usb's public
//! API, so one vendored module beats carrying a fork of the whole crate, and
//! the divergence stays checkable:
//!
//! ```text
//! diff ~/.cargo/registry/src/*/embassy-usb-0.6.0/src/class/cdc_ncm/mod.rs \
//!      src/cdc_ncm/mod.rs
//! ```
//!
//! `embassy_net.rs` beside this file is likewise unmodified apart from the
//! same macro qualification.
...
impl<'d> Handler for Control<'d> {
    fn set_alternate_setting(&mut self, iface: InterfaceNumber, alternate_setting: u8) {
        ...
        match alternate_setting {
            ALTERNATE_SETTING_ENABLED => defmt::info!("ncm: interface enabled"),
            ALTERNATE_SETTING_DISABLED => defmt::info!("ncm: interface disabled"),
            _ => unreachable!(),
        }
    }
    ...
    fn get_string(&mut self, index: StringIndex, _lang_id: u16) -> Option<&str> {
        if index == self.mac_addr_string {
            ...
            defmt::warn!("unknown string index requested");
            None
        }
    }
}
    ...
    pub fn new(
        ...
        alt.descriptor(
            ...
            &[
                ...
                // which this class does not implement -- `control_out` stalls
                // the request along with every other one it does not know.
                //
                // The claim is deliberate. macOS has a bug, Apple's own
                // r.170072016: an NCM device declaring no capabilities gets
                // SET_INTERFACE(alt=1) and then SET_INTERFACE(alt=0) a few
                // milliseconds later, about half the time, and the interface
                // is never enabled again. Claiming the bit puts the host on a
                // different path -- it sends SetEthernetPacketFilter, this
                // stalls it, and the fallback macOS already has for that
                // failure carries on correctly.
                //
                // Measured on teensy-bridge, which stalls the request exactly
                // as this does: failures fell from roughly one attachment in
                // three to one in fifty, with no regression on Windows 11,
                // Linux, iOS or iPadOS.
                1,
            ],
        );
    ...
    pub async fn read_packet(&mut self, buf: &mut [u8]) -> Result<usize, EndpointError> {
        ...
        loop {
            ...
            let Some(nth) = ntb.get(..12) else {
                defmt::warn!("Received too short NTB");
                continue;
            };
            let sig = u32::from_le_bytes(nth[0..4].try_into().unwrap());
            if sig != SIG_NTH {
                defmt::warn!("Received bad NTH sig.");
                continue;
            }
            ...
            let Some(datagram) = ntb.get(datagram_index..datagram_index + datagram_len) else {
                defmt::warn!("NDP has a datagram pointer out of range.");
                continue;
            };
